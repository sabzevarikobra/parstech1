...
@section('head')
    ...
    <style>
        ...
        .btn-save-preview {background: #2563eb; color: #fff; font-weight: 500; border-radius: 8px; padding: 1rem 0; border: none;}
    ...
    <div class="product-create-flex">
        ...
        <aside class="product-preview" id="productPreview">
            ...
            <div class="preview-image" id="preview-image">
                @if($product->image)
                    <img src="{{ asset('storage/' . $product->image) }}" alt="تصویر محصول">
                @else
            ...
            <div class="preview-info">
                <div class="info-group"><span class="info-label">نام:</span><span id="preview-name">{{ $product->name }}</span></div>
                <div class="info-group"><span class="info-label">کد:</span><span id="preview-code">{{ $product->code }}</span></div>
                <div class="info-group"><span class="info-label">قیمت فروش:</span><span id="preview-price">{{ number_format($product->sell_price) }}</span></div>
                <div class="info-group"><span class="info-label">موجودی:</span><span id="preview-stock">{{ $product->stock }}</span></div>
            </div>
            <div class="preview-actions">
                <button type="submit" form="product-form" class="btn-save-preview"><i class="fas fa-save"></i>&nbsp;ذخیره تغییرات</button>
        ...
        <main class="main-content-modern">
            <div class="main-section-title"><i class="bi bi-pencil-square ms-2"></i>ویرایش محصول</div>
            ...
            <form id="product-form" action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name-input" class="form-control" required value="{{ old('name', $product->name) }}">
                    </div>
                    <div class="col-md-6">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="sell_price" id="sell-price-input" class="form-control" value="{{ old('sell_price', $product->sell_price) }}" required>
                    </div>
                </div>
                <div class="row mb-4">
                    <div class="col-md-4">
                        <label class="form-label">موجودی</label>
                        <input type="number" name="stock" id="stock-input" class="form-control" value="{{ old('stock', $product->stock) }}">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">حداقل موجودی هشدار <span class="text-danger">*</span></label>
                        <input type="number" name="min_stock" class="form-control" value="{{ old('min_stock', $product->min_stock) }}" min="1" required>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="tab-content-modern active" id="tab1">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            @if($product->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $product->image) }}" class="img-thumbnail" alt="تصویر محصول" style="max-width:140px;">
                                </div>
                            @endif
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="hidden" name="gallery[]" id="gallery-input" value="{{ is_array($product->gallery) ? implode(',', $product->gallery) : '' }}">
                            @if($product->gallery && is_array($product->gallery))
                                <div class="d-flex flex-wrap mt-2">
                                    @foreach($product->gallery as $img)
                                        <img src="{{ asset('storage/'.$img) }}" class="rounded m-1" style="width:70px;height:70px;object-fit:cover;">
                                    @endforeach
                                </div>
                            @endif
                        </div>
                        <div class="col-md-6 mt-4">
                            ...
                            @if($product->video)
                                <div class="mb-2">
                                    <video src="{{ asset('storage/'.$product->video) }}" controls style="max-width:140px;"></video>
                                </div>
                            @endif
                ...
                <div class="tab-content-modern" id="tab2">
                    <div class="row mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="number" name="buy_price" class="form-control" value="{{ old('buy_price', $product->buy_price) }}">
                        </div>
                        <div class="col-md-4">
                            ...
                            <div id="attributes-area">
                                ...
                                @if($product->attributes && is_array($product->attributes))
                                    @foreach($product->attributes as $attr)
                                        <div class="row mb-2 attr-row">
                                            <div class="col-md-5">
                                                <input type="text" name="attributes[{{ $attrIndex }}][key]" class="form-control" placeholder="عنوان ویژگی" value="{{ $attr['key'] ?? '' }}">
                                            </div>
                                            <div class="col-md-5">
                                                <input type="text" name="attributes[{{ $attrIndex }}][value]" class="form-control" placeholder="مقدار" value="{{ $attr['value'] ?? '' }}">
                                            </div>
                                            <div class="col-md-2">
                ...
                <div class="tab-content-modern" id="tab4">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="barcode" class="form-control" id="barcode-input" value="{{ old('barcode', $product->barcode) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">بارکد فروشگاهی</label>
                            <input type="text" name="store_barcode" class="form-control" id="store-barcode-input" value="{{ old('store_barcode', $product->store_barcode) }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="tab-content-modern" id="tab5">
                    <div class="mb-4">
                        ...
                        <div class="row" id="shareholder-list">
                            @php
                                $productShareholders = $product->shareholders->pluck('pivot.percent','id')->toArray();
                            @endphp
                                        ...
                                        <div class="input-group-prepend">
                                            <div class="input-group-text">
                                                <input type="checkbox"
                                                    ...
                                                    @if(array_key_exists($shareholder->id, $productShareholders)) checked @endif
                                                >
                                            </div>
                                        </div>
                                        <input type="number"
                                            ...
                                            value="{{ $productShareholders[$shareholder->id] ?? '' }}"
                                            @if(!array_key_exists($shareholder->id, $productShareholders)) disabled @endif
                                        >
                ...
                <div class="text-end mt-4">
                    <button type="submit" class="btn btn-success btn-lg px-4">ذخیره تغییرات</button>
...
@section('scripts')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let attrCount = attrArea.querySelectorAll('.attr-row').length;
        ...
        attrArea.querySelectorAll('.remove-attribute').forEach(btn => {
